...
3. Planner's problem: weighing economic efficiency against favorable votes
...
- Bond measure went on ballot in 2008, 52.6% approved
...
- [$a(i, s)$]{.bh} non-monetary preference for CHSR
...
[Worker's problem]{.bh .invert} $v_{\omega}(i, j_C, m_C, j_L, m_L, s) =$
$$
\begin{aligned}
\max_{C, H_C, T_L}
B(i, s)
...
[Commuting costs]{.bh .invert} $d_{k}(i, j, m, s) = D_{k}(i, m) \tau_{k}(i, j, m, s)^{\rho} \quad \text{for } k = C, L$
...
[Productivity spillovers]{.bh .invert}
...
[Equilibrium productivity]{.bh .invert}
$$
...
## Estimating voting response to projected real income gains
...
- $\epsilon_{W,1}(i)$: voters' expectation error (voter's error)
  ...
  - Instrument $\ln \hat{W}_{19}(i)$ with $\ln \hat{W}_{08}(i)$ to get rid of this term
...
- Stations moved closer to population centers
  - where votes are harder to sway due to their high level of support
  - dense regions gain even more
...
## Can political preference explain I-5 route rejection?
...
- Given this estimated costs, consider an apolitical planner's optimal plan under I-5 route
...
- A first step towards the political economy of transport infrastructure
  - Combined a quantitative spatial model and simplified political incentives
  - Sensitive to mis-specification
- Highly reduced-form approximation of the political process
  - Role of state legislators & legislative bargaining
  - Local NIMBYism (enters via planning costs)
  - Partisanship & ideology (maximize party votes in the next general election)
  - Lobbying
