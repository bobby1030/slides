...
Outside choice (decreasing interest in reading newspapers)
...
- Endogenous:
  - Content quality:
    - nonadvertising space
    - \# staff for opinion section
    - \# of reporters
  - Local news ratio
  - Variety: HHI of share of staff in each section
- Exogenous: distance to newspaper's home county
...
Consider a Cobb-Douglas (hence log-linear) form of advertising demand:
...
In a two-stage game, a firm $j$ decides on:
...
- Average costs per copy:
  $$
  a c_{j}^{(q)} = \left(\gamma_{1} + \gamma_{2}f_{j} + \gamma_{3} \underbrace{\big(x_{1j} + a_{j}\big)}_{\text{pages}}\right) \underbrace{\log{(Q_{j})^{\gamma_{4}}}}_{\text{decreasing in total qty}} + \omega_{j}
  $$
- Ad profit: 
  $$
  \left(r_j - \underbrace{(1 + 1 / \lambda_{2}) \bigl(\overline{{{\zeta}}}+\zeta_{j} \bigr)}_{\text{MC of ad sales}}\right) \cdot \underbrace{a_j}_{\text{convex in qty}}
  $$
...
- Demand system: Newspaper demand, display ads demand
- Equilibrium conditions: FOC of the profit maximization problem
  - advertising rate
  - price
  - editorial decisions (characteristics)
...
Underestimated welfare loss for consumers and gain for firms if we do not consider the endogenous product characteristics.
...
- Negative changes in reader's surplus due to mergers are in markets where
  - higher newspaper penetration
  - higher overlap of merged newspapers prior to the merger
  - merged newspapers's sizes are comparable
  - market structure is duopoly